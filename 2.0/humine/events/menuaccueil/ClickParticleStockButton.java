...
public class ClickParticleStockButton implements Listener{
	...
	public void onClick(ClickItemMenuAccueilEvent event) {
		if(event.getItem().isSimilar(ItemShop.itemParticleStock())) {
	...
	{
		shopper.getStock().setCosmetique(shopper.getCosmetiques(TypeCosmetique.PARTICLE));
		if(shopper.getStock().isEmpty())
			MainShop.sendMessage(shopper.getPlayer(), "Particle Stock vide");
		else
