...
public class RemoveMoney implements CommandExecutor
{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
	{
		...
		if(args[0].equalsIgnoreCase("humis")) {
			...
			MainShop.sendMessage(sender, args[2] + " " + MainShop.getInstance().getBankHumis().getNameValue() + " supprim�s du compte de " + args[1]);
		}
		else {
			MainShop.getInstance().getBankPixel().removeMoney(args[1], Integer.parseInt(args[2]));
			MainShop.sendMessage(sender, args[2] + " " + MainShop.getInstance().getBankPixel().getNameValue() + " supprim�s du compte de " + args[1]);
		}
